...
def enhance_image(input_path: str, output_path: str):
    ...
    enhanced_img = enhancer.enhance(2.0)  # Tăng độ nét lên gấp đôi
    enhanced_img.save(output_path)

# ==== LỆNH /start ====
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Chào Huy~Onii! Gõ /hd để làm nét ảnh nhé! (⁠≧⁠▽⁠≦⁠)")

# ==== LỆNH /hd ====
async def hd_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Gửi ảnh bạn muốn làm nét đi nè~")
...
def main():
    ...
    print("Bot đang chạy rồi đó Huy~Onii...")
    app.run_polling()
